...
const UNSAFE_INTEGER = 9007199254740992
...
const fns = [
  ...
  { fn: () => randomNumber({ min: 0, max: 255 }), runs, expect: t => t >= 0 && t <= 255 },
  { fn: () => randomNumber({ min: 0, max: 257 }), runs, expect: t => t >= 0 && t <= 257 },
  { fn: () => randomNumber({ min: 0, max: 65537 }), runs, expect: t => t >= 0 && t <= 65537 },
  { fn: () => randomNumber({ min: 0, max: 16777217 }), runs, expect: t => t >= 0 && t <= 16777217 },
  { fn: () => randomNumber({ min: 0, max: 4294967297 }), runs, expect: t => t >= 0 && t <= 4294967297 },
  { fn: () => randomNumber({ min: 0, max: 1099511627776 }), runs, expect: t => t >= 0 && t <= 1099511627776 },
  { fn: () => randomNumber({ min: -3, max: -1 }), runs, expect: t => t >= -3 && t <= -1 },
  { fn: () => randomNumber({ min: 2, max: 1 }), runs, expect: t => t >= 1 && t <= 2 },
  { fn: () => randomNumber({ min: -5, max: 281474976710655 }), runs, expect: false },
  { fn: () => UNSAFE_INTEGER, expect: t => t > Number.MAX_SAFE_INTEGER },
  { fn: () => randomNumber({ min: 32000323, max: UNSAFE_INTEGER }), expect: false },
